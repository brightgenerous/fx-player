...
* Y○utube（※一文字伏せ字）の動画のURLを記述したプレイリストのファイルでもなるだけ再生するようにしました
* Y○utubeのPlaylist/Channel/UserのURLでもなるだけ再生するようにしました
...
テキストボックスにY○utubeのPlaylist、Channel、UserのURLを直接入力（してenter）すると、そのページの動画をプレイリストとして読み込みます
